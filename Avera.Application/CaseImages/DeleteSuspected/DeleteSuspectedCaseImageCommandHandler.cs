...
{
    internal sealed class DeleteSuspectedCaseImageCommandHandler
    (
    ...
    ) : ICommandHandler<DeleteSuspectedCaseImageCommand>
    {
        public async Task<Result> Handle(DeleteSuspectedCaseImageCommand command, CancellationToken cancellationToken)
        {
            ...
            var CaseImageToDelete = selectedCase?.CaseImages
                .FirstOrDefault(ci => ci.Type == Domain.Application.CaseImages.ImageType.Suspected 
            ...
            logger.LogInformation("Deleted Suspected Case Image with Id: {ImageId} for CaseId: {CaseId}", CaseImageToDelete!.Id, command.CaseId);
