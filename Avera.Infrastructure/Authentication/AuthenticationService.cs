...
namespace Avera.Infrastructure.Authentication
{
        ...
        ) : IAuthenticationService
    {
        ...
        public Task<Result> JoinInviteCodeAsync(Guid userId, string inviteCode, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<Result> RegisterAsync(RegisterRequest request, CancellationToken cancellationToken = default)
        {
            ...
            {
                Email = request.Email,
                UserName = request.FirstName.Split(" ")[0],
        ...
        {
            Logger.Information("Starting password reset for user {UserId}", userId);

            var user = await _userManager.FindByIdAsync(userId.ToString());

            if (user is null)
            {
                Logger.Warning("Password reset failed because user {UserId} was not found", userId);
                return Result.Failure(UserErrors.UserNotFound);
            }

            Logger.Information("Resetting password for user {UserId}", userId);
            IdentityResult? result = await _userManager.ResetPasswordAsync(user, token, password);

            if (!result.Succeeded)
            {
                Logger.Warning("Password reset failed for user {UserId}: {Errors}", userId, result.Errors.Select(e => e.Description));
                return HandleIdentityResult(result);
            }

            Logger.Information("Password reset completed successfully for user {UserId}", userId);
            return Result.Success();
        }
